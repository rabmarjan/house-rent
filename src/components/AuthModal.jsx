...
const AuthModal = ({ isOpen, onClose, onAuthSuccess }) => {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      authUtils.setUserData(response.user || response.agent, userType)

      // Call success callback
      if (onAuthSuccess) {
        onAuthSuccess(response.user || response.agent, userType)
      }
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      if (userType === 'agent') {
        userData.license_number = signUpData.license_number
        userData.years_experience = parseInt(signUpData.years_experience) || 0
      }
      ...
      if (userType === 'user') {
        ...
        response = await authAPI.registerAgent(userData)
      }
      ...
      authUtils.setUserData(loginResponse.user || loginResponse.agent, userType)

      // Call success callback
      if (onAuthSuccess) {
        onAuthSuccess(loginResponse.user || loginResponse.agent, userType)
      }
